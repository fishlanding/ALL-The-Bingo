...
# 红
tag Final_Feast add bingo_red
execute as @a[tag=bingo_red] run join red
# 黄
tag Aside_ing add bingo_yellow
execute as @a[tag=bingo_yellow] run join yellow
# 蓝
tag OneMiku add bingo_blue
execute as @a[tag=bingo_blue] run join blue
# 绿
tag fishing886_ add bingo_green
execute as @a[tag=bingo_green] run join green
# 旁观
tag UpSGE add bingo_gray
execute as @a[tag=bingo_gray] run join gray